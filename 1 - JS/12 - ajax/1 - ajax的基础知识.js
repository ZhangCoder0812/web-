/* 
   ajax，axios，$ajax，fetch 的区别？
     ...
     - ajax，axios，$ajax 核心是XMLHTTPRequest
       fetch 是es6新增的Api，浏览器新提供的一种和服务器通信的机制，默认也是基于promise管理
     - ajax是原生api操作
       axios是基于promise封装的ajax库
       $ajax是基于回调的方式封装的ajax库

  ajax：async javascript and xml
       异步的js指的不是js异步编程，指的是页面局部刷新。
    
    作用：实现 数据请求+客户端渲染 达到局部刷新
        ...
        - open(method,url,async,...)
            ...
            + 请求的地址 
                ...
                2. get会产生缓存「浏览器默认产生 不可控的缓存」，两次及以上 请求相同的地址 且参数也相同，浏览器
                   可能会把第一次的请求结果返回。
                   解决缓存：添加随机数 "./data.json&_"+Math.Random()
