...
function update() {
    ...
    phi = THREE.Math.degToRad(90 - lat); // 弧度转角度
    theta = THREE.Math.degToRad(lon);
    target.x = 500 * Math.sin(phi) * Math.cos(theta); // 500 任意写的 表示这个方向上的向量长度
    target.y = 500 * Math.cos(phi);
    target.z = 500 * Math.sin(phi) * Math.sin(theta);
    camera.lookAt(target); // 让相机看向这个位置
...
function move(e) {
    ...
    lat = (e.touches[0].pageY - startY) * 0.2 + startLon;
    lon = (startX - e.touches[0].pageX) * 0.2 + onPointerDownLon;
    lat = (c.touches[0].pageY - startY) * 0.2 + onPointerDownLat
    update();
}
