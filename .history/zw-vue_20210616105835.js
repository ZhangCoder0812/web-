...
3. nextTick 的原理？
   ...
   vue中dom更新是异步的，一旦观察到数据变化，会把同一个evevt loop中观察数据变化的watcher
   推送进这个队列，在下一次事件循环时 vue清空异步队列 进行dom更新。
   vue中开启异步任务优先级 Promise.then > MutationObserver > setImmediate > setTimeout
       
   vm.xxx = '123' dom并不会马上更新 而是在异步队列被清除的时候才会更新dom
  
   一般什么时候用到nextTick?
    + 在数据变化之后执行某个操作 获取节点
    <template>
      <div v-if='flag' ref='xxx'></div>
    </template>
