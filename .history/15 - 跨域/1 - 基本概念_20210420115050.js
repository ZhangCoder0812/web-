...
/*
 ...
 1. 修改本地host文件 [只适合本地开发解决跨域，上线后还是不行。]
 ...
 3. cors 在服务端处理请求允许的源，端口，请求方式...等等
        ...
        + 设置多个源不能连写 ...-Origin','http://127.0.0.1:5500,http://127.0.0.1:8080')
            ...
            app.use((req, res, next) => {
                ...
                res.header('Access-Control-Allow-Origin', origin) // 不在白名单中就设置空字符串 不允许这个源跨域
                res.header('Access-Control-Allow-Credentials', true)
                res.method === 'OPTIONS' ? res.sed('ok') : next() // 试探性请求不往下走
            })

 4. proxy 。webpack环境下使用，原理基于webpack-dev-server。
        客户端发送的请求会先发到webpack-dev-server启动的服务器上，webpack-dev-server做个代理
        再往真实服务器上发送请求。服务器和服务器之前不存在跨域。
            {
                devServer: {
                    proxy: {
                        '/api': {
        ...
        node模拟 proxy。node中发送请求再返回给客户端
            const request = require('request') node发送请求模块 要下载
            app.get('/test',(req,res)=>{
                let url = 'http:www.xxx.com/test'
                let res = request(url)
                res.pipe(res).pipe()
            })
            app.use(express.static('./')) 启动node打开本地index.html页面
